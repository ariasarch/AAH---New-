...
from PyQt5.QtCore import Qt, QObject, QTimer, QThread, pyqtSignal, pyqtSlot, Q_ARG, QMetaObject
import numpy as np
...
import warnings
warnings.filterwarnings("ignore", category=DeprecationWarning) # gets rid of warning due to pyqt version to py mismatch
...
class Processing_HUB(QObject):
    ...
    def __init__(self):
        ...
        self.controller = DisplayVideoController()
        self.controller.moveToThread(self.controller_thread)
        self.controller.updateFrame.connect(self.handle_update_frame)
        self.controller_thread.start()
        self.start_timer_signal.connect(self.controller.start_timer_slot)
        self.stop_timer_signal.connect(self.controller.stop_timer_slot)
    ...
    def get_initial_video(self):
        if self.video_path is not None and len(self.video_path) > 0:
            ...
            self.video_thread_seq()
        else:
            print('Failed to get video path')

    def video_thread_seq(self):
        if self.data_array is not None and len(self.data_array) > 0:
                self.play_video_instance = Play_video(self.data_array, self)
    ...
    def handle_update_frame(self, qimage):
        ...
        if self.current_function_index in [2,3]:
            new_qimage = self.frame_altering_function(frame=qimage)
        elif self.current_function_index == 4 and self.stop_frame_alt==0:
            self.pause_thread_play()
            if self.deglowed is not None:
                self.data_array=self.deglowed
            else:
                self.data_array= self.data_array_orig
            self.play()
            self.stop_frame_alt+=1
        ...
        if new_qimage is None:
            ...
            print('Failed to get frame from handle')
    ...
    def apply_changes_2(self):  # 
        ...
        if self.current_function_index>=5:
                ...
                if self.A is not None:
                    ...
                    print("Seed A has been passed", self.A)
        ...
        self.save_xarray_instance=save_xarray_instance
        if self.save_xarray_instance.seeds is not None:
            self.seeds=self.save_xarray_instance.seeds
        if self.save_xarray_instance.seeds_pnr is not None:
            self.pnr=self.save_xarray_instance.seeds_pnr
        if self.save_xarray_instance.seeds_ks is not None:
            self.ks=self.save_xarray_instance.seeds_ks
        if hasattr(self.save_xarray_instance, 'A_proj') and self.save_xarray_instance.A_proj is not None:
            self.A=self.save_xarray_instance.A_proj
            print("self.A obtained by HUB")
        if hasattr(self.save_xarray_instance, 'A_proj') and self.save_xarray_instance.A_proj is not None:
            self.C=self.save_xarray_instance.C_proj
        if save_xarray_instance.data_array is not None:
            self.data_array = self.save_xarray_instance.data_array
        else:
    ...
    def remove_glow(self): ### Causes seeds init to crash for unknown reason
        if self.data_array is not None and len(self.data_array) > 0:
            ...
            if self.deglow and self.play_video_instance is not None:
                self.pause_thread_play()
                print("The data is this before deglow", type(self.data_array))  
                # note 0.7 can be made into a slider at a later date if required
                self.data_array = self.data_array_orig.sel(frame=slice(None)) - 0.7*self.data_array_orig.min(dim='frame')  # applies deglow 
                self.deglowed=self.data_array
                self.play()   
                print("The data is this after deglow", type(self.data_array))  
        else:
...
# Controls the video thread
class DisplayVideoController(QObject):
    start_timer = pyqtSignal()
    stop_timer = pyqtSignal()
    updateFrame = pyqtSignal(QImage)

    def __init__(self):
        super(DisplayVideoController, self).__init__()
        self.timer = QTimer()
    ...
    def handle_timeout(self):
        if self.worker is not None:
            self.worker.get_video()

    def start_timer_slot(self):
        print("Starting timer")
        QMetaObject.invokeMethod(self.timer, "start", Qt.QueuedConnection, Q_ARG(int, int(self.worker.time_frame * 1000)))
...
class Play_video(QThread):
    ...
    request_stop_timer = pyqtSignal()

    def __init__(self, data_array, hub):
        super(Play_video, self).__init__()
        self.hub = hub
        self.controller = hub.controller
        self.controller.set_worker(self)
        self.request_start_timer.connect(self.hub.start_timer)
        self.request_stop_timer.connect(self.hub.stop_timer)
        self.data_array = data_array
        self.frame_rate = 5
        self.time_frame = 1 / self.frame_rate
        self.stop_thread = False
        self.convert_2_contours = False
        self.frame_index=0 # For get_video progress
    ...
    def get_video(self):
        if self.data_array is not None and self.ThreadActive:
            if self.frame_index < self.data_array.values.shape[0] - self.frame_rate:
                if not self.stop_thread:
                        img = self.data_array[self.frame_index].values
                        if img is not None:
                            ...
                            print('Failed to get frame in Play loop')
                else:
                    print('Thread stopped')
                    self.request_stop_timer.emit()
        else:
        ...
        def __init__(self,data_array, function_index=0, parameters=None, Seed_related_files=None):
            ...
            if Seed_related_files is not None:
                ...
                if hasattr(Seed_related_files, 'A'):
                    self.A_proj=Seed_related_files['A']
                print('seeds from A have been passed to save_changes', self.A_proj)
                if hasattr(Seed_related_files, 'C'):
                    self.C_proj=Seed_related_files['C']
...
### need to update functions from here down to align with VSC_Minian
        ...
        def pnr_refine_wrapper(self, frame): 
            if hasattr(self, 'seeds'):
                ...
                if pnr is not None:
                    self.seeds_pnr = pnr
                    self.seeds=seeds
                    print('After PNR refinement there are this many seeds: ' + str(len(self.seeds)))
                else:
        ...
        def initA_wrapper(self, frame):
            if  hasattr(self, 'seeds'):
                self.A_proj = vp.initA(self.seed_array, self.seeds[self.seeds["mask_mrg"]], self.value_2, int(self.value_3), self.value) 
                print(type(self.A_proj))
                self.initC_wrapper(frame)
            return frame

        def initC_wrapper(self, frame):
            if  hasattr(self,'self.A_proj'):
                self.C_proj = vp.initC(self.data_array, self.A_proj)
                print(self.A_proj, self.C_proj)
            return frame

        def unit_merge_wrapper(self, frame): # May have to run for both initC and initA
            print(type(self.A_proj), self.A_proj)
            if hasattr(self, 'A_proj') and self.A_proj is not None:
                self.A_proj, self.C_proj = vp.unit_merge(self.A_proj,self.C_proj,{'thres_corr': self.value})
...
### Dont need to pass the following functions frames therefore may deal with them seperately without calling loop
        def update_spatial_wrapper(self):
            if hasattr(self, 'A_proj') and self.A_proj:
                parameters={'dl_wnd':self.value_2,'sparse_penal': self.value, 'size_thres': self.value_3}
                self.A_proj = vp.update_spatial(self.data_array, self.A_proj, self.noise_fft, parameters)
            else:
        ...
        def update_temporal_wrapper(self):
            ...
            if self.C_proj is not None:
                self.C_proj = vp.update_temporal(self.A_proj, self.C_proj, parameters)
            else:
