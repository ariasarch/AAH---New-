...
import numpy as np
import video_processing_9 as vp
...
class Processing_HUB(QObject):
    ...
    def __init__(self):
        ...
        self.decimal_tenths_2=np.array([5,7,12,15])
...
class Initial_file_opening(QThread):
    ...
    def frame_array_2_xarray(self):
        ...
        self.get_chunk() #not currently being used
        self.data_array2=self.data_array
        self.data_array= self.data_array.chunk({"frame": self.chunk_comp["frame"], "height": -1, "width": -1})
        seeds_init_array=self.data_array2.chunk({"frame": -1, "height": self.chunk_comp["height"], "width": self.chunk_comp["width"]})
...
class Save_changes_2_xarray(QThread):
        ...
        def denoise(self):
            ...
            if self.value is None:
                self.value='gaussian'
            denoise_parameters={'method':'gaussian', 'ksize':kernel_size}
            return vp.denoise(self.data_array, **denoise_parameters)
        ...
        def estimate_motion(self):
                ...
                self.motion_vector = vp.estimate_motion(self.data_array)
                return vp.apply_transform(self.data_array, self.motion_vector) # applies motion transform
...
### need to update functions from here down to align with VSC_Minian
        ...
        def pnr_refine_wrapper(self, frame): 
            if hasattr(self, 'seeds'):
                refined_seeds = vp.pnr_refine(self.data_array, self.seeds, self.value, self.value_2)
                if refined_seeds is not None:
                    self.seeds_pnr = refined_seeds
        ...
        def ks_refine_wrapper(self, frame):
            if hasattr(self, 'seeds'):
                self.seeds_ks = vp.ks_refine(self.data_array, self.seeds, self.value)
        ...
        def seeds_merge_wrapper(self, frame): # Need to pass both self.seeds_ks and self.seeds_pnr
            if hasattr(self, 'seeds'):
                if self.seeds_ks:
                    seeds_final=self.seeds_ks
                    if self.seeds_pnr:
                        seeds_final = [self.seeds_pnr & self.seeds_ks].reset_index(drop=True)
                    self.max_proj=self.data_array.max("frame")
                    self.seeds = vp.seeds_merge(self.data_array,self.max_proj, seeds_final, self.value_3, self.value_2, self.value)
            else:
...
class MainWindow(QDialog):
 ...
 ### KS Refine, and all 'Update' functions need divide by 10
        ...
        self.decimal_tenths_2=np.array([5,6,7,12,15])
