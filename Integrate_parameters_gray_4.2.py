...
import numpy as np
import video_processing_11 as vp
...
### need to update functions from here down to align with VSC_Minian
        ...
        def seeds_merge_wrapper(self, frame): # Need to pass both self.seeds_ks and self.seeds_pnr
            if hasattr(self, 'seeds'):
                if self.seeds_ks:
                    ...
                    if self.seeds_pnr:
                        seeds_final = self.seeds[self.seeds_pnr["mask_pnr"] & self.seeds_ks["mask_ks"]].reset_index(drop=True)
                    self.max_proj=self.data_array.max("frame")
                    self.seeds = vp.seeds_merge(self.seed_array,self.max_proj, seeds_final, self.value_3, self.value_2, self.value)
            else:
        ...
        def initA_wrapper(self, frame):
            if  hasattr(self, 'seeds'):
                self.A = vp.initA(self.seed_array, self.seeds, self.value_2, int(self.value_3), self.value) # This is in original but doesn't work here [self.seeds["mask_mrg"]]
                print(type(self.A))
