...
def estimate_motion(
    ...
    if loop_dims:
        ...
        for lab in itt.product(*loop_labs):
            ...
            vmax, motion = est_motion_part(va.data, npart, chunk_nfm, **kwargs)
            ...
            res_dict[lab] = motion.assign_coords(**{k: v for k, v in zip(loop_dims, lab)})
        motion = xrconcat_recursive(res_dict, loop_dims)
    else:
        vmax, motion = est_motion_part(varr.data, npart, chunk_nfm, **kwargs)
        if kwargs.get("mesh_size", None):
            motion = xr.DataArray(
                motion,
                dims=[dim, "shift_dim", "grid0", "grid1"],
